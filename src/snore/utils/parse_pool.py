"""Spawn-context ProcessPoolExecutor dedicated to device-data parsing.
...
worker depending on the parser modules imported.  On machines with less than
~500 MB free, set ``SNORE_PARSE_MAX_WORKERS=1`` to cap the pool at one
worker process.

Separation rationale
--------------------
This pool is intentionally separate from the shared compute pool in
``process_pool.py``.  ResMed and OSCAR parsers submit all night futures
up-front; sharing a pool with analysis compute would starve analysis jobs
while a large import is in flight.  Giving parsing its own pool lets both
workloads make forward progress concurrently.
"""
...
def get_pool() -> ProcessPoolExecutor:
    """Return the parse ``ProcessPoolExecutor``, creating it on first call.
    ...
    return _pool_core.get_pool(
        ...
        logger=logger,
        broken_message=(
            "Parse process pool is broken; creating replacement pool. "
...
def shutdown_pool(*, wait: bool = False) -> None:
    """Shut down the parse pool and clear the singleton.
    ...
    _pool_core.shutdown_pool(
        _STATE,
        logger=logger,
        error_message="Error shutting down parse process pool",
        wait=wait,
    )
