"""Health command group — Apple Health import, summaries, and ingest tokens."""
...
@health.group("token")
def health_token() -> None:
    """Manage Apple Health ingest tokens."""
    pass


@health_token.command("create")
@click.option("--label", default=None, help="Optional label for this token")
@db_option
@actor_options
def token_create(
    label: str | None,
    ...
    """Mint a new ingest token for Health Auto Export push."""

    async def _run() -> None:
        from snore.auth.factory import resolve_cli_profile_id  # noqa: PLC0415
        from snore.services.health_token_service import (
            HealthTokenService,  # noqa: PLC0415
        )
        ...
        async with open_db_session(db) as session:
            ...
            svc = HealthTokenService(session)
            plaintext, info = await svc.create_token(profile_id, label=label)

        console.print(f"\nToken ID: {info.id}")
        if info.label:
            console.print(f"Label:    {info.label}")
        console.print(f"\nToken: {plaintext}\n")
        console.print("Store this token now — it will not be shown again.")
        console.print(
            "Send it in the X-SNORE-Ingest-Token header when pushing "
            "Health Auto Export data to snore serve."
        )

    asyncio.run(_run())


@health_token.command("list")
@db_option
@actor_options
def token_list(
    db: str | None,
    ...
    """List ingest tokens for this profile."""
    ...
    async def _run() -> None:
        ...
        async with open_db_session(db) as session:
            ...
            svc = HealthTokenService(session)
            tokens = await svc.list_tokens(profile_id)
        ...
        print_table(
            ...
            (
                (
                    str(t.id),
                    t.label or "—",
                    f"{t.created_at:%Y-%m-%d %H:%M}",
                    f"{t.last_used_at:%Y-%m-%d %H:%M}" if t.last_used_at else "—",
                    f"{t.revoked_at:%Y-%m-%d %H:%M}" if t.revoked_at else "—",
                )
                for t in tokens
            ),
            wide=False,
...
def token_revoke(
    ...
    async def _run() -> bool:
        ...
        async with open_db_session(db) as session:
            ...
            svc = HealthTokenService(session)
            return await svc.revoke(token_id, profile_id)

    ok = asyncio.run(_run())
    if ok:
        print_success(f"Token {token_id} revoked")
    else:
        raise click.ClickException(f"Token {token_id} not found or already revoked")
