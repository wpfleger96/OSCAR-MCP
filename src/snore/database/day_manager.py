...
class DayManager:
    ...
    # ------------------------------------------------------------------
    # Sync shims — for volatile import surfaces (importers.py,
    # import_service.py) that are being restructured in PR-1 and still
    # use a sync Session.  Do NOT use these in new code; prefer the async
    # class methods above.
    # ------------------------------------------------------------------

    @classmethod
    def get_or_create_day_sync(
        cls,
        device_id: int,
        day_date: date,
        db_session: Session,
    ) -> Day:
        """Sync shim of ``get_or_create_day`` for volatile importer path."""
        day = (
            db_session.execute(
                select(Day).filter_by(device_id=device_id, date=day_date)
            )
    ...
    @classmethod
    def _aggregate_day_statistics_sync(cls, day: Day, db_session: Session) -> None:
        """Sync shim of ``_aggregate_day_statistics`` for volatile importer path."""
        sessions = (
            db_session.execute(
                select(SessionModel)
                .filter_by(day_id=day.id, enabled=True)
