...
3. **Quality-flag data contract (pass-1 F3, executable per pass-2 F3'):** extend `RawSessionBlobs`/`AnalysisInputs` with an optional leak channel + session/device metadata for provenance. Per-flag executable derivations, each versioned + nullable with reason codes:
   ...
   - `ramp_active` — settings-driven timed heuristic (`VALIDITY_FLAGS_ALGO_VERSION = "v1"`), computed from the per-session KV settings `ramp_enabled` / `ramp_time` / `smart_ramp`. `ramp_enabled` unknown → `null` + `not_available`. SmartRamp → `null` + `smart_ramp_indeterminate` (SmartRamp ends on sleep detection, not a timer — a timed heuristic would fabricate data). Ramp disabled → `false`. Ramp enabled but `ramp_time` unknown → `null` + `not_available`. Otherwise the breath is in ramp iff its offset from the start of its CONTAINING persisted mask-on segment (ResMed re-runs ramp on every mask-on restart) is `< ramp_time * 60`; when segments are unknown, the offset is measured from session start.
   - `mask_off` — derived from the persisted `Session.mask_on_segments` intervals (recorded at import from the per-mask-on EDF segment structure; single-segment sessions store `[[0.0, duration]]`). Segments unknown (OSCAR imports, data imported before the column existed) → `null` + `segments_unknown`. Otherwise `true` iff the breath interval overlaps a mask-off gap (the complement of the segments within `[0, session_duration]`) with positive measure — breaths cannot lie wholly inside gaps (no flow samples there), so `true` marks the rare seam-spanning artifact breath and the dominant honest value is `false`. Never inferred from leak.
