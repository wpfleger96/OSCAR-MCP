...
def _create_session_for_day(db_session: DbSession, device: Device, day: Day) -> Session:
...
class TestDayServiceList:
    ...
    def test_list_returns_all_days(self, db_session, test_device):
        """All days returned when no filters applied."""
        _create_day(db_session, test_device, date(2025, 1, 1), ahi=2.0)
        _create_day(db_session, test_device, date(2025, 1, 2), ahi=3.0)
        _create_day(db_session, test_device, date(2025, 1, 3), ahi=4.0)
        db_session.flush()

        service = DayService(db_session)
        items, total = service.list_days(limit=0)

        assert total == 3
        assert len(items) == 3

    def test_list_sorted_desc_by_date(self, db_session, test_device):
        """Results are sorted by date descending."""
        _create_day(db_session, test_device, date(2025, 1, 1))
        _create_day(db_session, test_device, date(2025, 1, 3))
        _create_day(db_session, test_device, date(2025, 1, 2))
        db_session.flush()

        service = DayService(db_session)
        items, _ = service.list_days(limit=0)

        assert items[0].date == date(2025, 1, 3)
        assert items[1].date == date(2025, 1, 2)
        assert items[2].date == date(2025, 1, 1)

    def test_list_with_from_date_filter(self, db_session, test_device):
        """from_date filter excludes older records."""
        _create_day(db_session, test_device, date(2025, 1, 1))
        _create_day(db_session, test_device, date(2025, 1, 5))
        _create_day(db_session, test_device, date(2025, 1, 10))
        db_session.flush()

        service = DayService(db_session)
        items, total = service.list_days(from_date=date(2025, 1, 5), limit=0)

        assert total == 2
        assert all(item.date >= date(2025, 1, 5) for item in items)

    def test_list_with_to_date_filter(self, db_session, test_device):
        ...
        service = DayService(db_session)
        items, total = service.list_days(to_date=date(2025, 1, 5), limit=0)

        assert total == 2
        assert all(item.date <= date(2025, 1, 5) for item in items)

    def test_list_with_date_range(self, db_session, test_device):
        """Combined from_date/to_date range filters correctly."""
        _create_day(db_session, test_device, date(2025, 1, 1))
        _create_day(db_session, test_device, date(2025, 1, 5))
        _create_day(db_session, test_device, date(2025, 1, 10))
        _create_day(db_session, test_device, date(2025, 1, 15))
        db_session.flush()
    ...
    def test_list_with_device_id_filter(self, db_session, test_device):
        ...
        _create_day(db_session, test_device, date(2025, 1, 1))
        _create_day(db_session, other_device, date(2025, 1, 2))
        db_session.flush()

        service = DayService(db_session)
        items, total = service.list_days(device_id=test_device.id, limit=0)

        assert total == 1
        assert items[0].device_id == test_device.id

    def test_list_limit_applied(self, db_session, test_device):
        """limit parameter restricts number of items returned."""
        for i in range(10):
            _create_day(db_session, test_device, date(2025, 1, 1) + timedelta(days=i))
        db_session.flush()

        service = DayService(db_session)
        items, total = service.list_days(limit=3)

        assert total == 10
        assert len(items) == 3

    def test_list_offset_applied(self, db_session, test_device):
        ...
        service = DayService(db_session)
        items_page1, total = service.list_days(limit=2, offset=0)
        items_page2, _ = service.list_days(limit=2, offset=2)
    ...
    def test_list_item_fields(self, db_session, test_device):
        """DayListItem contains expected fields with correct values."""
        _create_day(
            db_session, test_device, date(2025, 6, 15), ahi=2.5, total_therapy_hours=7.0
        )
        db_session.flush()

        service = DayService(db_session)
        items, _ = service.list_days()

        assert len(items) == 1
        item = items[0]
        assert item.date == date(2025, 6, 15)
        assert item.device_id == test_device.id
...
class TestDayServiceGet:
    ...
    def test_get_existing_day(self, db_session, test_device):
        ...
        _create_day(
            db_session, test_device, date(2025, 4, 10), ahi=3.5, total_therapy_hours=8.0
        )
        db_session.flush()

        service = DayService(db_session)
        result = service.get_day(date(2025, 4, 10))

        assert result is not None
        assert result.date == date(2025, 4, 10)
        assert result.device_id == test_device.id
    ...
    def test_get_day_with_session_ids(self, db_session, test_device):
        """DayDetail includes linked session IDs."""
        day = _create_day(db_session, test_device, date(2025, 5, 20))
        sess1 = _create_session_for_day(db_session, test_device, day)
        ...
        service = DayService(db_session)
        result = service.get_day(date(2025, 5, 20))

        assert result is not None
        assert set(result.session_ids) == {sess1.id, sess2.id}

    def test_get_day_empty_session_ids(self, db_session, test_device):
        """DayDetail has empty session_ids when no sessions linked."""
        _create_day(db_session, test_device, date(2025, 7, 1))
        db_session.flush()

        service = DayService(db_session)
        result = service.get_day(date(2025, 7, 1))
